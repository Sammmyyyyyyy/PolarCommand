export function calculateExpeditionRisk(cargoList, inventoryList, assetsList, stations) {
    // 1. Cargo Risk (0 - 30 max weight)
    const delayedOrCriticalCargo = cargoList.filter((c) => c.status === 'Delayed' || c.priority === 'CRITICAL' || c.delayHours > 0);
    ...
    for (const s of stations) {
        if (s.weather.windSpeed > 40)
            weatherRiskScore += 5;
        else if (s.weather.windSpeed > 30)
            weatherRiskScore += 2;
    }
