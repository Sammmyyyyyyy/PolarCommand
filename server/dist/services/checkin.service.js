...
export class CheckInService {
    static async recordCheckIn(expeditionId, personnelId, data, user) {
        const person = await prisma.personnel.findUnique({
            where: { id: personnelId },
        });
        if (!person)
            throw new Error(`Personnel member ${personnelId} not found`);
        const now = new Date();
    ...
    static async evaluateOverdueCheckIns(expeditionId) {
        const now = new Date();
    ...
    static async getPersonnelAccountability(expeditionId) {
