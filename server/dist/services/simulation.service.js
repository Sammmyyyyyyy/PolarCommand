...
export class SimulationService {
    static async runSimulation(params) {
        ...
        if (scenarioType === 'cargo-delay') {
            ...
            const targetCargo = sandboxCargo.find((c) => c.id === params.cargoId) || sandboxCargo[0];
            if (targetCargo) {
                targetCargo.delayHours += delay;
                targetCargo.status = 'Delayed';
                // Check linked inventory
                const linkedInv = sandboxInventory.find((i) => i.linkedCargoId === targetCargo.id || i.category.toLowerCase() === targetCargo.category.toLowerCase()) || sandboxInventory[0];
                let prevDays = 12;
                let newDays = 8;
                if (linkedInv) {
                ...
                impactChain.push({ step: 1, title: 'Simulation Trigger', description: `Consignment ${targetCargo.cargoCode} delayed by +${delay} hours in maritime corridor.`, status: 'trigger' }, { step: 2, title: 'Arrival Slip', description: `Coastal offload slips downstream by +${Math.round(delay / 24)} days.`, status: 'cascade' }, { step: 3, title: 'Reserve Depletion', description: `${impactedStationName} inventory buffers drop from ${prevDays} to ${newDays} days (safety threshold breached).`, status: 'breach' }, { step: 4, title: 'Risk Surge', description: `Expedition operational risk score surges from ${baselineRisk} to ${simulatedRisk}/100.`, status: 'alert' }, { step: 5, title: 'Actionable Mitigation', description: recommendation, status: 'recommendation' });
            }
        }
            ...
            impactChain.push({ step: 1, title: 'Surge Applied', description: `Station consumption elevated by ${mult}x due to extreme sub-zero weather.`, status: 'trigger' }, { step: 2, title: 'Accelerated Depletion', description: `Daily burn rate increases across fuel and heating provisions.`, status: 'breach' }, { step: 3, title: 'Operational Risk Spike', description: `System risk score elevates to ${simulatedRisk}/100.`, status: 'alert' }, { step: 4, title: 'Mitigation Formulated', description: recommendation, status: 'recommendation' });
        }
            ...
            impactChain.push({ step: 1, title: 'Mechanical Stoppage', description: 'Heavy snow vehicle track assembly seizure reported on ice traverse.', status: 'trigger' }, { step: 2, title: 'Transport Corridor Bottleneck', description: 'Overland resupply corridor between stations halted.', status: 'breach' }, { step: 3, title: 'Elevated Hazard', description: `Operational risk escalates to ${simulatedRisk}/100.`, status: 'alert' }, { step: 4, title: 'Recommended Protocol', description: recommendation, status: 'recommendation' });
        }
        else {
            ...
            impactChain.push({ step: 1, title: 'Disruption Event', description: `${scenarioType.replace('-', ' ').toUpperCase()} initiated.`, status: 'trigger' }, { step: 2, title: 'Risk Impact', description: `Expedition risk calculated at ${simulatedRisk}/100.`, status: 'alert' }, { step: 3, title: 'Recommendation', description: recommendation, status: 'recommendation' });
        }
    ...
    static async applyScenario(simulationId, user) {
        const sim = await prisma.simulationRecord.findUnique({ where: { id: simulationId } });
        if (!sim)
            throw new Error('Simulation record not found');
    ...
    static async discardScenario(simulationId) {
        await prisma.simulationRecord.update({
            where: { id: simulationId },
