...
export class WeatherService {
    static provider = new OpenMeteoWeatherProvider();
     ...
     */
    static calculateOperationalImpact(data) {
        ...
        const windChill = Math.round(13.12 + 0.6215 * temp - 11.37 * Math.pow(Math.max(1, windKmh), 0.16) + 0.3965 * temp * Math.pow(Math.max(1, windKmh), 0.16));
        let travelFeasibility = 'OPEN';
        ...
        const iceSurfaceFriction = wind > 35 ? 'Severe Glaze' : wind > 22 ? 'Roughed Ice' : 'Moderate';
        return {
     ...
     */
    static async getStationWeather(stationId, forceRefresh = false) {
     ...
     */
    static async recordStationWeather(stationId, data) {
