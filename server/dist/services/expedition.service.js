...
export class ExpeditionService {
    ...
    static async updateLifecycle(id, lifecycleStatus, user) {
    ...
    static async updateExpedition(id, data, user) {
    ...
    static async deleteExpedition(id, user) {
    ...
    static async getDashboardSummary(expeditionId) {
        ...
        const maintenanceDueAssets = expedition.assets.filter((a) => a.maintenanceInterval - a.operatingHours <= 0 || a.lifecycleStatus === 'MAINTENANCE_DUE' || a.status === 'Maintenance').length;
     ...
     */
    static async evaluateReadiness(expeditionId) {
        const exp = await prisma.expedition.findUnique({
            where: { id: expeditionId },
            include: {
                commander: true,
        ...
        const hasDoctor = exp.personnel.some((p) => p.role.toLowerCase().includes('doctor') ||
        ...
        const failedAssets = exp.assets.filter((a) => a.lifecycleStatus === 'FAILED' || a.status === 'Critical' || a.status === 'Unavailable');
        if (failedAssets.length > 0) {
            blockingIssues.push(`Critical assets unavailable or failed: ${failedAssets.map((a) => a.assetCode).join(', ')}`);
        }
        const maintenanceDue = exp.assets.filter((a) => a.lifecycleStatus === 'MAINTENANCE_DUE' || a.maintenanceInterval - a.operatingHours <= 0);
        ...
        const hasFuelOrFood = exp.cargo.some((c) => c.category.toLowerCase().includes('fuel') ||
        ...
        const commMethodConfigured = !!(exp.notes?.toLowerCase().includes('iridium') ||
     ...
     */
    static async checkResourceAvailability(expeditionIdOrOrgId, orgId, _startDate, _endDate) {
        ...
        const committedPersonnel = new Map();
