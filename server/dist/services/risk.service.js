    ...
    static async calculateAndRecordExpeditionRisk(expeditionId) {
        ...
        for (const inv of expedition.inventory) {
            ...
            if (daysOfSupply <= inv.safetyThresholdDays) {
                ...
                contributingFactors.push(`${inv.station.name} ${inv.itemName} stock (${Math.round(daysOfSupply)} days) below safety threshold (${inv.safetyThresholdDays} days)`);
        ...
        // Total Calculation
        const totalScore = Math.min(100, normalizedCargo + normalizedInventory + normalizedAssets + normalizedIncidents + normalizedPersonnel + normalizedWeather);
        let riskLevel = 'LOW';
        if (totalScore >= 70)
            riskLevel = 'CRITICAL';
