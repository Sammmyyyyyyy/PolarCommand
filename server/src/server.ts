...
async function getDefaultExpeditionId(): Promise<string> {
...
app.get('/api/dashboard', async (req, res) => {
  try {
    const expId = (req.query.expeditionId as string) || (await getDefaultExpeditionId());
...
app.get('/api/cargo', async (req, res) => {
  try {
    const expId = await getDefaultExpeditionId();
    const cargo = await CargoService.listCargo(expId, req.query as any);
    res.json(cargo);
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
