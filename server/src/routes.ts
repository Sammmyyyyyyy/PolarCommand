...
// Expeditions
apiRouter.get('/expeditions', (_req: Request, res: Response) => {
...
apiRouter.get('/expeditions/:id', (req: Request, res: Response) => {
  ...
  if (!item) return res.status(404).json({ error: 'Expedition not found' });
  res.json(item);
});

// Stations
apiRouter.get('/stations', (_req: Request, res: Response) => {
...
// Cargo
apiRouter.get('/cargo', (_req: Request, res: Response) => {
  res.json(stateEngine.getCargo());
});

apiRouter.get('/cargo/:id', (req: Request, res: Response) => {
  const id = String(req.params.id);
  const item = stateEngine.getCargoItem(id);
  if (!item) return res.status(404).json({ error: 'Cargo shipment not found' });
  res.json(item);
});

// Hero Simulation: Simulate Cargo Delay
apiRouter.post('/cargo/:id/simulate-delay', (req: Request, res: Response) => {
  const id = String(req.params.id);
...
apiRouter.get('/inventory/:stationId', (req: Request, res: Response) => {
  const stationId = String(req.params.stationId);
  res.json(stateEngine.getInventory(stationId));
});

// Assets
apiRouter.get('/assets', (_req: Request, res: Response) => {
...
// Personnel
apiRouter.get('/personnel', (_req: Request, res: Response) => {
  res.json(stateEngine.getPersonnel());
});

apiRouter.get('/personnel/:id', (req: Request, res: Response) => {
  const id = String(req.params.id);
  const item = stateEngine.getPersonnelMember(id);
  if (!item) return res.status(404).json({ error: 'Personnel member not found' });
  res.json(item);
});
...
// What-If Simulation
apiRouter.post('/simulations', (req: Request, res: Response) => {
  const result = stateEngine.runWhatIfSimulation(req.body);
  res.json(result);
});

// Resource Optimization (Cargo allocation)
apiRouter.post('/optimization/cargo-allocation', (req: Request, res: Response) => {
...
apiRouter.post('/hero-action', (_req: Request, res: Response) => {
